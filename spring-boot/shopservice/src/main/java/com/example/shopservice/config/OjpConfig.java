 ...
 * which is not the case here (we use spring.datasource.catalog.url / checkout.url).
 * Registering the bridge manually ensures that ojp.* properties declared in
 * application.properties are forwarded to JVM system properties before any DataSource
 * bean is created, making them available to the OJP JDBC driver.
 */
@Configuration
