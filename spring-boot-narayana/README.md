...
- **Narayana JTA Transaction Manager** - For coordinating distributed transactions
...
This application uses Narayana with **connection pooling disabled** as required. The XA datasources are configured directly without an intermediate connection pool, allowing OJP (Open JDBC Proxy) to manage connection pooling at the proxy level.

### Important Notes
- Connection pooling is disabled - OJP XA DataSource is used directly
- Narayana manages XA transactions without pooling overhead
- This configuration is compatible with OJP's requirement to manage the pool in the proxy
...
```
src/main/java/com/example/narayana/
├── NarayanaApplication.java                # Main application class
├── config/
...
src/test/java/com/example/narayana/integration/
...
The application uses Narayana to coordinate XA transactions across two separate PostgreSQL databases:
...
- **PostgreSQL Database 2**: Secondary entity manager for AuditLog entities
  - Configured with `max_prepared_transactions=10` to enable XA support
- **Narayana Transaction Manager**: Coordinates 2PC across both databases

Each datasource is configured directly using OJP's XA DataSource without connection pooling.
...
- **Currently configured for Java 25** for development and testing.
